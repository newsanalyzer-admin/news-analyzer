...
ON CONFLICT (official_name) DO UPDATE SET org_type = EXCLUDED.org_type, updated_at = NOW();

-- Independent Agencies
INSERT INTO government_organizations (id, official_name, acronym, org_type, branch, website_url, mission_statement, created_at, updated_at)
VALUES
    ('dddddddd-dddd-dddd-dddd-dddddddddddd', 'Environmental Protection Agency', 'EPA', 'independent_agency', 'executive', 'https://www.epa.gov', 'To protect human health and the environment', NOW(), NOW()),
    ('eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee', 'National Aeronautics and Space Administration', 'NASA', 'independent_agency', 'executive', 'https://www.nasa.gov', 'To explore space for the benefit of humanity', NOW(), NOW()),
    ('ffffffff-ffff-ffff-ffff-ffffffffffff', 'Central Intelligence Agency', 'CIA', 'independent_agency', 'executive', 'https://www.cia.gov', 'To collect, analyze, and disseminate intelligence', NOW(), NOW())
ON CONFLICT (official_name) DO UPDATE SET org_type = EXCLUDED.org_type, updated_at = NOW();
